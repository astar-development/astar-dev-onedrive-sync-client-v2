...
public sealed class OneDriveGraphClientShould
{
    ...
    public async Task ReturnSuccessResponse()
    {
        ...
        OneDriveGraphClient client = CreateClient(handler, telemetry);
    ...
    private static OneDriveGraphClient CreateClient(HttpMessageHandler handler, IOneDriveGraphTelemetry telemetry)
    {
        HttpClient httpClient = new(handler) { BaseAddress = new Uri("https://graph.microsoft.com/v1.0/") };
        return new OneDriveGraphClient(httpClient, telemetry);
    }
